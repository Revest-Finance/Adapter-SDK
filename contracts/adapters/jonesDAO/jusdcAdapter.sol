 ...
 */
contract jusdcAdapter is PermissionedAdapter, ERC4626 {
    ...
    constructor(ERC20 _asset, address _adapter, address _router, address _vault, address _feeHelper, address _whitelist, 
    address _uvrt, address _dust_wallet) ERC4626(_asset, "jUSDC 4626 Adapter", "JUSDC_RSN8") {
        adapter = _adapter;
        router = _router;
        vault = _vault;
        ...
        _asset.safeApprove(_adapter, type(uint256).max);
    }

    function totalAssets() public view virtual override returns (uint256) {
        uint totalShares = ERC20(vault).balanceOf(address(this));

        return IJonesFeeCalculator(feeHelper).getUSDCRedemption(totalShares, address(this));
    }
    ...
    function afterDeposit(uint256 assets, uint256 shares) internal virtual override {
        IJonesAdapter(adapter).depositStable(assets, true);
    }
    ...
    function _withdraw(
        ...
        require(IJonesFeeCalculator(feeHelper).getUSDCRedemption(jonesShares, address(this)) >= amount, "invalid redemption amount");


        uint beforeTokenBal = asset.balanceOf(address(this));
        _doWithdrawal(shares, jonesShares, sender);

        // the change in the number of assets from _doWithdrawal
    ...
    function _doWithdrawal(
        ...
        IJonesRouter(router).stableWithdrawalSignal(jonesShares, true);
    }
        ...
        returns (uint256 assets)
    {
        ...
        assets = IJonesFeeCalculator(feeHelper).getUSDCRedemption(jonesShares, address(this));
    }
    ...
    function convertJonesSharesToAssets(uint jonesShares) internal view returns (uint assets) {
        ...
        return supply == 0 ? jonesShares : jonesShares * (ERC4626(uvrt).totalAssets()) / supply;
    }
    ...
    function convertAssetsToJonesShares(uint assets) internal view returns (uint) {
        ...
        uint totalUVRT = ERC4626(vault).totalAssets();
        uint totalJonesAssets = IJonesFeeCalculator(feeHelper).sharesToUSDC(totalUVRT);
    ...
    function calculateAssetsPlusFee(uint assets) internal view returns (uint adjustedAmount) {
        ...
        uint retention = IJonesWhitelist(whitelist).getRoleInfo(RESONATE_ROLE).jUSDC_RETENTION;
 ...
 /*///////////////////////////////////////////////////////////////
    ...
    function maxDeposit(address) public view virtual override returns (uint256) {
        ...
        if (IJonesAdapter(adapter).useFlexibleCap()) {

            IJonesGlpStableVault stableVault = IJonesAdapter(adapter).stableVault();

            //Note: Both in 1e18 precision but the methods do the conversions for us so it should be ok
            uint tvl = stableVault.tvl();
            uint cap = IJonesAdapter(adapter).getUsdcCap();
    ...
    function maxWithdraw(address owner) public view virtual override returns (uint256) {
        ...
        //The users' owned percent of the jUSDC
        uint jusdcShares = convertSharesToJonesShares(balanceOf[owner]);

        //How much that corresponds to after fees
        return IJonesFeeCalculator(feeHelper).getUSDCRedemption(jusdcShares, address(this));
    }
